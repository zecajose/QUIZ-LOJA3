...
export default function Page() {
  ...
  const calcHealthScore = () => {
    ...
    if (pd >= 5) score += 20; else if (pd >= 3) score += 14; else if (pd >= 1) score += 8
    ...
    if (cv >= 4) score += 10; else if (cv >= 2) score += 6; else if (cv >= 1) score += 3
    ...
    else if (['20% a 40%','60% a 80%'].includes(formData.vendasRedesSociais)) score += 4
    else score += 2
  ...
  const generateDiagnostic = () => {
    ...
    if (['Não posto regularmente','Esporadicamente'].includes(formData.frequenciaPostagens))
      out.push({ area:'Engajamento', status:'Crítico', message:'Consistência de posts é prioridade.', color:'text-red-600 bg-red-50' })
    if (formData.investimentoAnuncios === 'Nunca investi')
      out.push({ area:'Marketing Pago', status:'Oportunidade', message:'Teste anúncios básicos com orçamento baixo.', color:'text-blue-600 bg-blue-50' })
    if (!formData.diferencial || formData.diferencial === 'Ainda não tenho um diferencial claro')
      out.push({ area:'Posicionamento', status:'Atenção', message:'Defina um ponto único e comunique em bio e criativos.', color:'text-yellow-600 bg-yellow-50' })
    return out
  }
          ...
          <div className="bg-white rounded-2xl shadow-xl p-6 md:p-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
              ...
              <div className="p-5 rounded-xl border bg-gray-50">
                ...
                <div className="flex items-center justify-between text-sm mb-2 text-gray-600">
                  <span>Status</span><span className={`${health.color} font-semibold`}>{health.label} • {health.pct}%</span>
                </div>
                <div className="w-full bg-gray-200 rounded-full h-2">
            ...
            <div className="text-center space-y-4">
              ...
              <p className="text-sm text-gray-500">Investimento único de R$ 17,90 • Acesso imediato • Garantia de 7 dias</p>
    ...
    <div className="min-h-screen bg-gradient-to-br from-pink-50 to-purple-50 p-4">
      <div className="max-w-2xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <div className="w-full bg-gray-200 rounded-full h-2">
            <div className="bg-gradient-to-r from-pink-500 to-purple-600 h-2 rounded-full transition-all duration-300" style={{ width: `${((currentStep + 1) / questions.length) * 100}%` }} />
          </div>
        </div>

        <div className="bg-white rounded-2xl shadow-xl p-6 md:p-8 mb-6">
          <div className="flex items-center mb-6">
            <div className="bg-gradient-to-r from-pink-500 to-purple-600 p-3 rounded-xl text-white mr-4">{questions[currentStep].icon}</div>
            <h2 className="text-xl md:text-2xl font-semibold text-gray-800">{questions[currentStep].title}</h2>
          </div>
          ...
          <div className="space-y-4">
            {questions[currentStep].type === 'text' && (
              <input
                ...
                value={formData[questions[currentStep].id] as string}
                onChange={(e) => updateFormData(questions[currentStep].id, e.target.value)}
            ...
            {questions[currentStep].type === 'select' && (
              <div className="space-y-3">
                {(questions[currentStep] as any).options.map((opt: string) => (
                  <button
                    key={opt}
                    onClick={() => updateFormData(questions[currentStep].id, opt)}
                    className={`w-full p-4 text-left border rounded-xl transition-all duration-200 ${
                      (formData as any)[questions[currentStep].id] === opt
            ...
            {questions[currentStep].type === 'multiple' && (
              <div className="space-y-3">
                {(questions[currentStep] as any).options.map((opt: string) => {
                  const selected = ((formData as any)[questions[currentStep].id] as string[]).includes(opt)
                  return (
                    <button
                      key={opt}
                      onClick={() => toggleArrayValue(questions[currentStep].id, opt)}
                      className={`w-full p-4 text-left border rounded-xl transition-all duration-200 ${
